...
function addImageToAccount(currentAcc, src) {
    if(!currentAcc.images.includes(src))
        {
...
submitButton.addEventListener('click', () => {
    ...
    if (validateEmail()) {
        ...
        if (!slidersPanel.classList.contains("sliders-panel-visible")) {
            slidersPanel.classList.add("sliders-panel-visible");
            setTimeout(()=>{addButton.scrollIntoView({behavior:"smooth"})},600);
        ...
        // image.src = newSrc;
        addImageToAccount(currentAccount, newSrc);
       // newSrc = changeImage();
